...
// Set to true if you want to drop existing table and recreate (erases all data)
define('FORCE_RECREATE_TABLE', false);

// Set to true to insert sample application records
...
try {
    ...
    addMessage("Checking/Creating database `" . DB_NAME . "`...");
    ...
    // Step 4: Create table `applications`
    $tableExists = $pdo->query("SHOW TABLES LIKE 'applications'")->rowCount() > 0;
    
    if ($tableExists && FORCE_RECREATE_TABLE) {
        addMessage("Table `applications` exists. Dropping it (FORCE_RECREATE_TABLE = true)...", 'warning');
        ...
        if ($count == 0) {
            ...
            $sampleData = [
                ...
                [
                    ...
                    'program' => 'Engineering and Natural Science',
                    'school_name' => 'Prepa Tec',
                    'personal_statement' => 'My passion for renewable energy drives me to study sustainable engineering. WCU‚Äôs lab facilities are unmatched.',
...
<body>
<div class="container">
    <h1>üîß WCU Applications Database Setup</h1>
    <p>This script will create the database <strong><?php echo DB_NAME; ?></strong> and the <strong>applications</strong> table.</p>
    
    <?php if ($error): ?>
        <div class="alert">
            ‚ù?Setup encountered errors. Please check messages below and fix your configuration.
        </div>
    <?php else: ?>
        <div class="success-box">
            ‚ú?Setup completed without fatal errors.
        </div>
    <?php endif; ?>
    ...
    <hr>
    <div>
        <a href="apply.php" class="btn">üìù Go to Application Form</a>
        <a href="#" class="btn btn-secondary" onclick="window.location.reload();">‚ü?Run Again</a>
    </div>
    
    <div class="footer">
        <p>‚ö†Ô∏è For security, <strong>delete this file (install.php)</strong> after successful installation.</p>
